...
//----------------------------------------------------------SENDING KEY 
	...
	{
		*(pData + 0) 	= 0; // key length set to b"00" for 128-bit key length   
		*(pData + 5) 	= key32[0]; // key word in # 0
		*(pData + 0xC)	= key32[1]; // key word in, #1 
		*(pData + 0xD)  = key32[2]; // #2
		*(pData + 0xE)  = key32[3];//#3
...
//-----------------------------------------------------------------------
	...
	while( ! ( (*(pData+6))& 0x1 )   ){}//  wait until data_ready = '1'
	...
	uint64_t* unpacked_encrypted,uint64_t* key,int key_length,size_t i)
{
	...
	unpacked_encrypted[i] = cipherText[0];// store encrypted data
	unpacked_encrypted[i + 1] = cipherText[1];// accumulate the encrypted text
	FPGA_decrypt(key,cipherText,decryptedText,key_length); // decrypt data using the FPGA 
	unpacked_decrypted[i] = decryptedText[0];// store decrypted data 
...
void Simon_begin(int size, uint64_t* packet, uint64_t* key, int key_length)
{
	...
	SimonContext context;
	SIMON_init(&context, key, key_length); // start generating subkeyys 
